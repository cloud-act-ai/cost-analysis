...
COST_SCHEMA_FILE="data/cost_analysis_schema.json"
AVG_SCHEMA_FILE="data/avg_daily_cost_schema.json"
COST_DATA_FILE="data/cost_analysis_new_no_header.csv"
AVG_DATA_FILE="data/avg_daily_cost_no_header.csv"
...
if [ ! -f "$COST_DATA_FILE" ]; then
  ...
  python3 data/generate_updated_data.py --no-header
  if [ ! -f "$COST_DATA_FILE" ]; then
